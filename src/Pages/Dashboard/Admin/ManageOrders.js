...
const ManageOrders = () => {
  const {
    ...
    fetch("https://compact-server-side.onrender.com/manage_orders").then(
      (res) => res.json()
    )
  );
            ...
            {orders.map((order, index) => (
              <tr>
                ...
                <td>
                  ...
                  {order.price && order.paid && pending && (
                    <button
                      onClick={handlePending}
                      className="btn btn-sm btn-primary text-lg text-black hover:text-white"
                    >
                      PENDING
                    </button>
                  )}
