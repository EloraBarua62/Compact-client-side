...
const CheckoutForm = ({ order }) => {
  ...
  useEffect(() => {
    fetch("https://compact-server-side.onrender.com/create-payment-intent", {
      method: "POST",
      headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    if (intentError) {
      ...
      fetch(`https://compact-server-side.onrender.com/my_order/${_id}`, {
        method: "PATCH",
        headers: {
          "content-type": "application/json",
          authorization: `Bearer ${localStorage.getItem("accessToken")}`,
    ...
    <>
      ...
      {success && (
        <div>
          ...
          <p className="text-purple-700 text-sm font-bold py-2">
