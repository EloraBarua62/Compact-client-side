...
const stripePromise = loadStripe('pk_test_51L2FNjGToWrNOrcdY2rMMYbCnd2jvkmpCOER0IFoTbAW2cROPw23ivTXddaVL9cd5SgeSQ38lAwf9gXLChdK4gy500YHVNvhVA');

const Payment = () => {
    const { id } = useParams();
    const url = `https://shielded-castle-46219.herokuapp.com/my_order/${id}`;
    const { data: order, isLoading } = useQuery(['my_order', id], () => fetch(url, {
        method: 'GET',
        headers: {
